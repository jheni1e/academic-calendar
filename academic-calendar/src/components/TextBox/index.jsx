...
function TextBox({ label, isBigInput, hasError, errorText, hasWarning, warningText, isSearchInput, searchMet, isPasswordInput, type = "text", ...rest }, ref) {
    ...
    return (
        <div className={`a-text-field ${isPasswordInput ? 'a-text-field--password' : ''} ${isSearchInput ? 'a-text-field--search' : ''}`}>
            ...
            {isPasswordInput && (
                <button
                    type="button"
                    className="a-text-field__icon-password"
                    onClick={handlePasswordToggle}
                >
                    <i
                        className={`a-icon ${showPassword ? 'ui-ic-watch-off' : 'ui-ic-watch-on'}`}
